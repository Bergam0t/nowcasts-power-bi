...
dat2 <- lapply(unique(dat1$measure), function(z) {
  ...
  tmp2 <- ts(tmp1$num,
           start=c(as.numeric(format(tmp1$dates[1], "%Y")),
           as.numeric(format(tmp1$dates[1], "%j")))
           )
  
  #err_ets<-tsCV(tmp2,fets,h=1)
  #err_arima<-tsCV(tmp2,farima,h=1)

  if (mean((tsCV(tmp2, fets, h=1))^2, na.rm=TRUE) < 
      mean((tsCV(tmp2, farima, h=1))^2, na.rm=TRUE) ) {  # ts_ets < ts_arimax
  ...
  output_df <- data.frame(measure=tmp1$measure[1],
                          dates= seq.Date(min(tmp1$dates),
                                          by=1,
                                          length.out=length(c(tmp3$x, tmp3$mean))
                                          )) %>%
      mutate(ycentral=c(tmp3$x, tmp3$mean),
            ylower1=c(tmp3$x, tmp3$lower[,2]),
            ylower2=c(tmp3$x, tmp3$lower[,1]),
            yupper1=c(tmp3$x, tmp3$upper[,2]),
            yupper2=c(tmp3$x, tmp3$upper[,1])
            ) %>% 
...
p <- ggplotly(
    dat2 %>%
      ...
      ungroup() %>% 
      ggplot(aes(x=dates)) +
      # Add upper and lower limit fills
      ...
      # Add lower limit labels
      geom_text(aes(x=dates,y=as.numeric(ylower1a), label=ylower1a), 
                position=position_dodge(width=0.9), vjust=+1.5, size=2) +
      # Add upper limit labels
      geom_text(aes(x=dates,y=as.numeric(yupper1a), label=yupper1a), 
                position=position_dodge(width=0.9), vjust=-1, size=2) +
      # Add central line labels
      geom_text(aes(x=dates,y=as.numeric(ycentrala), label=ycentrala), 
                position=position_dodge(width=0.9), vjust=-0.8, size=2) +
      # Set up axis
